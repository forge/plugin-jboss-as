...
public class FilePermissions {
    ...
    public static FilePermissions of(final int mode) {
        ...
        return new FilePermissions(FilePermission.fromInt(parts[0]), FilePermission.fromInt(parts[1]), FilePermission.fromInt(parts[2]));
    }
    ...
    public static enum FilePermission {
        ...
        static FilePermission fromInt(final int i) {
            if (i < 0 || i > 7) {
                throw new IllegalStateException(String.format("The permissions value must be between 0 and 7. Value '%d' is invalid.", i));
            }
            return values()[i];
        }
